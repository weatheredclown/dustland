(() => {
    ...
    const dustlandEnc = (globalThis.Dustland ?? (globalThis.Dustland = {}));
    ...
    const labeled = (container, label, el) => {
        const labelEl = document.createElement('label');
        labelEl.textContent = label;
        container.appendChild(labelEl);
        container.appendChild(el);
    };
    const numberInput = (container, label, value) => {
    ...
    const optionSelect = (container, label, opts, current, placeholderText) => {
        ...
        return {
            render(container, state) {
                ...
                if (state.templateName)
                    nameEl.value = state.templateName;
    ...
    const lootStep = () => {
        ...
        return {
            render(container, state) {
                ...
                if (state.lootItemName)
                    nameEl.value = state.lootItemName;
    ...
    const encounterWizard = {
        ...
        commit(state) {
            ...
            if (isNew) {
                const combat = { HP: state.hp ?? 8, ATK: state.atk ?? 2, DEF: state.def ?? 0 };
                if (lootTable.length)
                    combat.lootTable = lootTable;
                templates.push({ id: templateId, name: newName, combat });
            }
            const entry = { map: state.map || 'world', templateId };
            if (state.zoneTag) {
                entry.mode = 'zone';
                ...
                if (typeof state.maxDist === 'number')
                    entry.maxDist = state.maxDist;
            }
            if (!isNew && lootTable.length)
                entry.lootTable = lootTable;
            const result = { encounters: [entry] };
            if (templates.length)
    ...
    const dustlandWizards = (dustlandEnc.wizards ?? (dustlandEnc.wizards = {}));
