...
const OFFICE_IMPL = (() => {
    ...
    const WORLD_MID = Math.floor(worldWidth / 2), WORLD_MIDY = Math.floor(worldHeight / 2);
    function liftHelmetCurse() {
        uncurseItem('cursed_vr_helmet');
    }
    function genForestWorld(seed = Date.now()) {
        setRNGSeed(seed);
        world = Array.from({ length: worldHeight }, () => Array.from({ length: worldWidth }, (_, x) => {
            if (x === WORLD_MID)
                return tileWater;
        ...
        if (creatorMapData.grid && creatorMapData.grid.length)
            interiors['creator'] = creatorMapData;
        buildings.length = 0;
    ...
    function makeFloor1() {
        ...
        addElevator(grid);
        // four pillars
        [6, FLOOR_W - 7].forEach((x) => [10, FLOOR_H - 7].forEach((y) => (grid[y][x] = tileWall)));
        // front desk
        for (let x = 3; x < FLOOR_W - 3; x++) {
            if (x !== midX)
                grid[FLOOR_H - 5][x] = tileWall;
        }
    ...
    function makeCastle() {
        const W = 30, H = 30;
        const grid = Array.from({ length: H }, () => Array.from({ length: W }, () => tileWall));
...
const officeModule = JSON.parse(DATA);
...
globalThis.startGame = function startGame() {
    ...
    if (!module)
        throw new Error('Malformed or incomplete module: OFFICE_MODULE');
