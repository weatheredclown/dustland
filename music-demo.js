...
(function () {
    ...
    var tempoEl = byId('tempo');
    var tempoLabel = byId('tempoLabel');
    var seedEl = byId('seed');
    var keyEl = byId('key');
    var moodList = byId('moodList');
    var moodInfo = byId('moodInfo');
    var nowStat = byId('nowStat');
    var barBeat = byId('barBeat');
    var scaleLabel = byId('scaleLabel');
    var mgBtn = byId('mgBtn');
    var mgStatus = byId('mgStatus');
    var toneBtn = byId('toneBtn');
    var toneStatus = byId('toneStatus');
    var fxPulse = byId('fxPulse');
    var fxPulseLabel = byId('fxPulseLabel');
    var fxBits = byId('fxBits');
    var fxBitsLabel = byId('fxBitsLabel');
    var fxDown = byId('fxDown');
    var fxDownLabel = byId('fxDownLabel');
    var fxEcho = byId('fxEcho');
    var fxEchoLabel = byId('fxEchoLabel');
    var downloadJsonBtn = byId('downloadJsonBtn');
    var loadJsonBtn = byId('loadJsonBtn');
    var loadJsonInput = byId('loadJsonInput');
    // Disable FX controls until Tone is ready
    [fxPulse, fxBits, fxDown, fxEcho]
        .forEach(function (el) { if (el)
        el.disabled = true; });
    var harmonyInput = byId('harmonyInput');
    var moodKeyEl = byId('moodKey');
    var moodBpmEl = byId('moodBpm');
    var moodScaleEl = byId('moodScale');
    var moodDensityEl = byId('moodDensity');
    var moodSwingEl = byId('moodSwing');
    var moodLeadWaveEl = byId('moodLeadWave');
    var moodBassWaveEl = byId('moodBassWave');
    var barStartEl = byId('barStart');
    var saveMoodBtn = byId('saveMoodBtn');
    var seedRows = byId('seedRows');
    var addSeedBtn = byId('addSeedBtn');
    var clearSeedBtn = byId('clearSeedBtn');
    // saveSeedBtn removed: motif edits auto-apply
    var motifList = byId('motifList');
    var motifModeEl = byId('motifMode');
    var addMotifBtn = byId('addMotifBtn');
    var dupMotifBtn = byId('dupMotifBtn');
    var delMotifBtn = byId('delMotifBtn');
    var jsonOut = byId('jsonOut');
    // --- Globals & Engine ---
    ...
    function euclid(steps, pulses, rot) {
        ...
        r(level);
        var rotN = ((rot || 0) % steps + steps) % steps;
    ...
    function mkOsc(type, freq, time) {
        ...
        o.type = type;
        if (freq != null)
            o.frequency.setValueAtTime(freq, time || ac.currentTime);
        return o;
    }
    ...
    function playKick(t, vel) {
        ...
        var g = e.gain;
        var k = ac.createGain();
        k.gain.setValueAtTime(((vel != null ? vel : 1) * 1.2), t);
    ...
    function playSnare(t, tight, vel) {
        ...
        var g = ac.createGain();
        g.gain.setValueAtTime(((tight ? 0.3 : 0.45) * (vel != null ? vel : 1)), t);
    ...
    function playHat(t, open, vel) {
        ...
        var g = ac.createGain();
        g.gain.setValueAtTime(((open ? 0.18 : 0.12) * (vel != null ? vel : 1)), t);
        s.connect(hp).connect(e.gain).connect(g).connect(master);
        s.start(t);
        s.stop(t + 0.15);
    ...
    function playBassNote(midi, t, dur, vel) {
        ...
        var g = ac.createGain();
        g.gain.setValueAtTime(0.3 * (vel != null ? vel : 1), t);
    ...
    function playLeadNote(midi, t, dur, vel) {
        ...
        var g = ac.createGain();
        g.gain.setValueAtTime(0.18 * (vel != null ? vel : 1), t);
    ...
    function playBarIntro(degree, t) {
        if (music.barStart === 'arp')
            playChordArp(degree, t);
        else if (music.barStart === 'stab')
            playChordStab(degree, t);
    }
    // Timing helpers
    ...
    function scheduleNote(step, t) {
        ...
        if (isKick)
            playKick(t2, vel);
        if (isSnare)
            playSnare(t2, false, vel);
        if (hatPattern[beatInBar])
            playHat(t2, (music.density > 0.8) && (beatInBar % 4 === 2), vel);
        ...
        var bassNote;
        if (beatInBar % 8 === 0)
            bassNote = root; // strong downbeats
        else if (beatInBar % 4 === 2)
            bassNote = fifth; // mid-beat accent
        else
            bassNote = root;
        ...
        if (override && override.length) {
            for (var i = 0; i < override.length; i++) {
                var ev = override[i];
                if (ev.step === stepInBar)
                    playLeadNote(ev.midi, t2, secondsPer16th() * (ev.durSteps || 2), vel);
            }
        }
            ...
            for (var k = 0; k < mev.length; k++) {
                var evm = mev[k];
                if (evm.step === stepInBar)
                    playLeadNote(evm.midi, t2, secondsPer16th() * (evm.durSteps || 2), vel);
            }
        }
    ...
    function scheduleTransitionFill(barStartTime) {
        ...
        // Snare 16th roll
        for (var i = 0; i < 8; i++)
            playSnare(t0 + i * secondsPer16th() * 0.5, true);
        // Noise riser
        var s = mkNoise();
        var g = ac.createGain();
    ...
    function applyMood(id) {
        ...
        if (tempoEl) {
            tempoEl.value = String(music.bpm);
        ...
        if (harmonyInput)
            harmonyInput.value = (m.harmony || []).join(',');
        if (barStartEl)
            barStartEl.value = music.barStart;
        if (moodKeyEl)
            moodKeyEl.value = music.key;
        if (moodBpmEl)
            moodBpmEl.value = String(music.bpm);
        if (moodScaleEl)
            moodScaleEl.value = music.scale;
        if (moodDensityEl)
            moodDensityEl.value = String(music.density);
        if (moodSwingEl)
            moodSwingEl.value = String(music.swing);
        if (moodLeadWaveEl)
            moodLeadWaveEl.value = music.leadWave;
        if (moodBassWaveEl)
            moodBassWaveEl.value = music.bassWave;
        loadSeedRows();
        renderConfig();
    }
    var mg = {
    ...
    function setMgStatus(text) { if (mgStatus)
        mgStatus.textContent = 'Magenta: ' + text; }
    ...
    function loadMagenta() {
        if (mg.loading || mg.ready) {
            if (!mg.loading)
                setMgStatus('ready');
            return;
        }
        mg.loading = true;
        setMgStatus('loading libs…');
    ...
    function scheduleMagentaInterpolationForBar(barIndex, nextMoodId) {
        ...
        try {
            if (window.mm && window.mm.sequences) {
                if (!a.quantizationInfo)
                    a = window.mm.sequences.quantizeNoteSequence(a, 4);
                if (!b.quantizationInfo)
                    b = window.mm.sequences.quantizeNoteSequence(b, 4);
            }
        }
        catch (e) { /* best-effort */
            void e;
        ...
        mg.vae.interpolate([a, b], 5)
            ...
            for (var i = 1; i < out.length - 1; i++) { // use interior sequences
                ...
                if (!seq.quantizationInfo && window.mm && window.mm.sequences) {
                    try {
                        seq = window.mm.sequences.quantizeNoteSequence(seq, 4);
                    }
                    catch (e) { /* ignore */
                        void e;
                    }
                ...
                for (var off = 0; off <= stepsPerBar; off += stepsPerBar) {
                    ...
                    if (ev && ev.length)
                        barEvents.push(ev);
                    if (barEvents.length >= 4)
                        break; // cap to 4 bars
                }
                if (barEvents.length >= 4)
                    break;
            }
            for (var k = 0; k < barEvents.length; k++)
                mg.leadOverride[barIndex + 1 + k] = barEvents[k];
    ...
    function nextNote() {
        ...
        if (current16th % stepsPerBar === 0) {
            ...
            var old = music.barCount - 2;
            if (mg.leadOverride[old])
                delete mg.leadOverride[old];
        }
    }
    function scheduler() {
        while (playing && nextNoteTime < ac.currentTime + scheduleAheadTime) {
            ...
            if (current16th % stepsPerBar === 0 && music.nextMood) {
                ...
                if (mg.ready)
                    scheduleMagentaInterpolationForBar(Math.floor(current16th / stepsPerBar), music.nextMood);
            }
    ...
    function start() {
        ...
        if (tone.enabled && window.Tone && !tone.ready) {
            ...
            Tone.start().then(function () { }).catch(function () { });
        }
    ...
    if (startBtn && seedEl) {
        startBtn.onclick = function () {
            if (!ac)
                setSeed(parseInt(seedEl.value || '1', 10));
            start();
        };
    }
    if (stopBtn)
    ...
    if (nextBtn)
        nextBtn.onclick = function () { if (playing)
            music.nextMood = music.nextMood || music.mood; };
    ...
    if (mgBtn)
        mgBtn.onclick = function () { loadMagenta(); };
    if (toneBtn)
        toneBtn.onclick = function () { loadTone(); };
    function loadTone() {
        if (tone.loading || tone.ready) {
            if (!tone.loading)
                toneStatus && (toneStatus.textContent = 'Tone: ready');
            return;
        }
        tone.loading = true;
        if (toneStatus)
            toneStatus.textContent = 'Tone: loading…';
        loadScript('https://cdn.jsdelivr.net/npm/tone/build/Tone.js')
            ...
            tone.enabled = true;
            if (toneStatus)
                toneStatus.textContent = 'Tone: ready';
            bindFxControls();
        })
            .catch(function (e) { tone.loading = false; if (toneStatus)
            toneStatus.textContent = 'Tone: failed (' + e.message + ')'; });
    }
    function bindFxControls() {
        ...
        // Enable controls
        [fxPulse, fxBits, fxDown, fxEcho].forEach(function (el) { if (el)
            el.disabled = false; });
        if (fxPulse)
            fxPulse.oninput = function () {
                var v = parseFloat(fxPulse.value || '0.5');
                if (fxPulseLabel)
                    fxPulseLabel.textContent = v.toFixed(2);
                try {
                    if (tone.synths && tone.synths.lead.oscillator && tone.synths.lead.oscillator.width)
                        tone.synths.lead.oscillator.width.value = v;
                }
                catch (e) { /* ignore */
                    void e;
                }
        ...
        if (fxBits)
            fxBits.oninput = function () {
                var bits = parseInt(fxBits.value || '5', 10);
                if (fxBitsLabel)
                    fxBitsLabel.textContent = bits + ' bits';
                try {
                    tone.fx.crusher.bits = bits;
                }
                catch (e) { /* ignore */
                    void e;
                }
        ...
        if (fxDown)
            fxDown.oninput = function () {
                var v = parseFloat(fxDown.value || '0.85');
                if (fxDownLabel)
                    fxDownLabel.textContent = v.toFixed(2);
                try {
                    var freq2 = 2000 + v * 12000;
                    tone.fx.color.frequency.setValueAtTime(freq2, Tone.now());
                }
                catch (e) { /* ignore */
                    void e;
                }
        ...
        if (fxEcho)
            fxEcho.oninput = function () {
                var v = parseFloat(fxEcho.value || '0.10');
                if (fxEchoLabel)
                    fxEchoLabel.textContent = v.toFixed(2);
                try {
                    tone.fx.ping.wet.value = v;
                }
                catch (e) { /* ignore */
                    void e;
                }
    ...
    if (loadJsonBtn && loadJsonInput) {
        ...
        loadJsonInput.onchange = function () {
            var file = loadJsonInput.files && loadJsonInput.files[0];
            if (!file)
                return;
            ...
            reader.onload = function () {
                try {
                    ...
                    if (Array.isArray(cfg.seedNotes)) {
                        globalSeed = cfg.seedNotes.slice();
                    }
                    ...
                    if (cfg.motifMode === 'repeat' || cfg.motifMode === 'improv')
                        motifMode = cfg.motifMode;
                    if (cfg.moods) {
                        MOODS.forEach(function (m) {
                            ...
                            if (mm) {
                                if (mm.key)
                                    m.key = mm.key;
                                if (typeof mm.bpm === 'number')
                                    m.bpm = mm.bpm;
                                if (mm.scale)
                                    m.scale = mm.scale;
                                if (typeof mm.swing === 'number')
                                    m.swing = mm.swing;
                                if (typeof mm.density === 'number')
                                    m.density = mm.density;
                                if (mm.leadWave)
                                    m.leadWave = mm.leadWave;
                                if (mm.bassWave)
                                    m.bassWave = mm.bassWave;
                                if (mm.barStart)
                                    m.barStart = mm.barStart;
                                if (Array.isArray(mm.harmony))
                                    m.harmony = mm.harmony.slice();
                            }
                        });
                    }
    ...
    function persistMoodFromInputs() {
        ...
        if (moodBpmEl) {
            m.bpm = music.bpm = parseInt(moodBpmEl.value, 10) || m.bpm;
            if (tempoEl) {
                tempoEl.value = String(music.bpm);
                tempoLabel.textContent = music.bpm + ' BPM';
            }
        ...
        if (moodDensityEl) {
            ...
            if (!isNaN(d))
                m.density = music.density = d;
        }
        if (moodSwingEl) {
            var s = parseFloat(moodSwingEl.value);
            if (!isNaN(s))
        ...
        renderConfig();
        if (moodInfo)
            moodInfo.textContent = 'Mood updated.';
    }
    function persistHarmonyFromInput() {
        ...
        if (arr.length) {
            ...
            renderConfig();
            if (moodInfo)
                moodInfo.textContent = 'Harmony set to [' + arr.join(',') + '] ' + (playing ? '(applies bar-by-bar)' : '');
        }
    }
    if (harmonyInput) {
    ...
    function renderMotifList() {
        if (!motifList)
            return;
        while (motifList.firstChild)
            motifList.removeChild(motifList.firstChild);
        for (var i = 0; i < motifs.length; i++) {
            var m = motifs[i];
            var btn = document.createElement('div');
            btn.textContent = m.name || ('Motif ' + (i + 1));
            btn.className = 'mood-btn' + (i === selectedMotifIndex ? ' active' : '');
            (function (idx) { btn.onclick = function () { selectedMotifIndex = idx; loadSeedRows(); renderMotifList(); if (moodInfo)
                moodInfo.textContent = 'Selected ' + (motifs[idx].name || ('Motif ' + (idx + 1))); }; })(i);
            // highlight currently playing motif
            if (i === activeMotifIndex) {
                btn.style.outline = '1px solid var(--accent)';
                btn.style.boxShadow = '0 0 8px rgba(158,247,160,.3)';
            }
    ...
    function setMotifMode(mode) {
        ...
        if (motifModeEl)
            motifModeEl.value = motifMode;
        if (moodInfo)
            moodInfo.textContent = (motifMode === 'repeat' ? 'Motif mode: Repeat selected' : 'Motif mode: Improvise between motifs');
    }
    function motifVariation(notes) {
        // Return a shallow-varied copy of notes within the bar
        var out = [];
        for (var i = 0; i < notes.length; i++)
            out.push({ step: (notes[i].step | 0), degree: (notes[i].degree | 0), dur: (notes[i].dur | 0) || 2 });
        if (out.length) {
            // 30%: nudge last note later by 1 step
            if (rnd() < 0.3) {
                out[out.length - 1].step = Math.min(stepsPerBar - 1, out[out.length - 1].step + 1);
            }
            // 25%: octave up last note
            if (rnd() < 0.25) {
                out[out.length - 1].degree += 7;
            }
        }
        return out;
    }
    function chooseActiveMotifForBar(barIndex) {
        ...
        if (motifMode === 'repeat') {
            ...
            if (activeMotifIndex < 0)
                activeMotifIndex = 0;
            else if (rnd() < 0.4) {
                var next = activeMotifIndex;
                var guard = 0;
                while (motifs.length > 1 && next === activeMotifIndex && guard++ < 8)
                    next = randi(0, motifs.length - 1);
    ...
    function addSeedRow(step, degree, dur) {
        ...
        s.type = 'number';
        s.placeholder = 'step';
        s.value = (step != null ? step : '');
        var d = document.createElement('input');
        d.type = 'number';
        d.placeholder = 'degree';
        d.value = (degree != null ? degree : '');
        var u = document.createElement('input');
        u.type = 'number';
        u.placeholder = 'dur';
        u.value = (dur != null ? dur : '');
    ...
    function readSeedRows() {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if (r.length >= 3) {
                var st = parseInt(r[0].value, 10);
                var dg = parseInt(r[1].value, 10);
                var du = parseInt(r[2].value, 10);
                if (!isNaN(st) && !isNaN(dg))
                    out.push({ step: st, degree: dg, dur: isNaN(du) ? 2 : du });
            }
        }
    ...
    function autoPersistMotifFromRows() {
        try {
            ...
            ensureDefaultMotif();
            if (selectedMotifIndex >= 0 && motifs[selectedMotifIndex])
                motifs[selectedMotifIndex].notes = notes.slice();
            // keep globalSeed in sync for backward compatibility
            globalSeed = notes.slice();
            renderConfig();
            if (moodInfo)
                moodInfo.textContent = 'Motif updated (' + notes.length + ' notes).';
        }
        catch (e) { /* ignore */
            void e;
        }
    ...
    function loadSeedRows() {
        if (!seedRows)
            return;
        while (seedRows.firstChild)
            seedRows.removeChild(seedRows.firstChild);
        var seed;
        ensureDefaultMotif();
        if (selectedMotifIndex >= 0 && motifs[selectedMotifIndex])
            seed = motifs[selectedMotifIndex].notes || [];
        else
            seed = globalSeed || [];
        for (var i = 0; i < seed.length; i++)
            addSeedRow(seed[i].step, seed[i].degree, seed[i].dur);
    }
    if (addSeedBtn)
        addSeedBtn.onclick = function () { addSeedRow('', '', ''); };
    if (clearSeedBtn)
        clearSeedBtn.onclick = function () { while (seedRows.firstChild)
            seedRows.removeChild(seedRows.firstChild); };
    ...
    if (addMotifBtn)
        addMotifBtn.onclick = function () {
            ...
            renderMotifList();
            renderConfig();
            autoPersistMotifFromRows();
        };
    if (dupMotifBtn)
        dupMotifBtn.onclick = function () {
            ensureDefaultMotif();
    ...
    if (delMotifBtn)
        delMotifBtn.onclick = function () {
            if (motifs.length <= 1)
                return; // keep at least one
            if (selectedMotifIndex < 0)
                return;
    ...
    if (motifModeEl)
        motifModeEl.onchange = function () { setMotifMode(motifModeEl.value === 'repeat' ? 'repeat' : 'improv'); renderConfig(); };
    if (saveMoodBtn)
        saveMoodBtn.onclick = function () { persistMoodFromInputs(); persistHarmonyFromInput(); };
    function renderConfig() {
        ...
        for (var j = 0; j < motifs.length; j++)
            motifsObj.push({ name: motifs[j].name || ('Motif ' + (j + 1)), notes: (motifs[j].notes || []).slice() });
        ...
        try {
            ...
            jsonOut.textContent = '{ /* error building config: ' + e.message + ' */ }';
        }
    }
    // Defaults
    applyMood('somber');
