type DoorCoordinate = { x: number; y: number };
...
const doorLinkerStep: WizardStepFactory = (
  ...
  let currentState: WizardState | undefined;
  ...
  return {
    render(container, state) {
      ...
      const createPane = (label: string, key: string) => {
    ...
    validate() {
      ...
      if (!isDoorCoordinate(currentState[entryKeyName])) return;
      if (!isDoorCoordinate(currentState[exitKeyName])) return;
      return true;
    },
    onComplete() {}
  };
