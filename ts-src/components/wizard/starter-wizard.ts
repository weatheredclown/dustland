interface StarterWizardState extends WizardState {
  moduleName?: string;
  seed?: string;
...
(() => {
  ...
  const snapToWalkable = (pos: { x: number; y: number }): { x: number; y: number } => {
    const g = globalThis as any;
    const world = g.world;
    const tile = g.TILE;
    if (!Array.isArray(world) || !world.length || !tile) return pos;
    const blocked = new Set([tile.WATER, tile.WALL, tile.BUILDING].filter((t: unknown) => typeof t === 'number'));
    const h = world.length;
    const w = world[0]?.length ?? 0;
    const inBounds = (x: number, y: number): boolean => x >= 0 && y >= 0 && x < w && y < h;
    const ok = (x: number, y: number): boolean => inBounds(x, y) && !blocked.has(world[y][x]);
  ...
  const seedStep = (): WizardStep<StarterWizardState> => {
    ...
    return {
      ...
      onComplete(state) {
        if (input) state.seed = input.value.trim();
      }
    };
  };

  const questStep = (): WizardStep<StarterWizardState> => {
    let itemEl: HTMLInputElement | null = null;
    ...
    return {
      render(container, state) {
        ...
        if (state.itemName) itemEl.value = state.itemName;
        container.appendChild(labelEl);
  ...
  const starterWizard: WizardDefinition<StarterWizardState> = {
    ...
    commit(state) {
      ...
      (globalThis as any).genWorld?.(seed);
  ...
  const dustlandWizards = (dustlandStarter.wizards ??= {});
