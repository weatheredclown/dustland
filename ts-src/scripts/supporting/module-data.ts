...
export interface ModuleSource {
  /** Project-relative POSIX path of the file the data came from. */
  file: string;
...
export function extractModuleData(text: string): string | null {
  const match = text.match(DATA_BLOCK_RE);
  return match ? match[1] : null;
}

/** Parse the module data baked into a .module.js file. */
export function readModuleData(file: string): Record<string, unknown> | null {
 ...
 */
export function listModuleSources(projectRoot: string): ModuleSource[] {
  ...
  if (fs.existsSync(modulesDir)) {
    for (const name of fs.readdirSync(modulesDir)) {
      ...
      if (name.endsWith('.module.js')) {
        ...
        if (data) sources.push({ file: toPosixPath(path.relative(projectRoot, file)), data });
