...
function renderNPCList() {
  ...
  Array.from(list.children).forEach(btn => {
    ...
    if (idx === editNPCIdx) {
      (btn as HTMLElement).style.outline = '2px solid var(--accent, #9ef7a0)';
...
function renderZoneList() {
  ...
  const encMap = document.getElementById('encMap') as HTMLSelectElement | null;
  if (encMap) {
    const encZone = document.getElementById('encZone') as HTMLSelectElement | null;
