...
document.getElementById('intLabel').addEventListener('input', e => {
  ...
  const btn = document.querySelector(`#intList button[data-idx="${editInteriorIdx}"]`);
  if (btn) btn.textContent = I.label || I.id;
  updateInteriorOptions();
  refreshChoiceDropdowns();
});
...
function renderNPCList() {
  ...
  Array.from(list.children).forEach(btn => {
    ...
    if (idx === editNPCIdx) {
      ...
      (btn as HTMLElement).scrollIntoView({ block: 'nearest' });
    }
  });
  updateQuestOptions();
  refreshChoiceDropdowns();
...
function renderArenaList() {
  ...
  Array.from(listEl.children).forEach(btn => {
    (btn as HTMLElement).onclick = () => editArena(parseInt((btn as HTMLElement).dataset.idx, 10));
  });
}
...
function renderZoneList() {
  ...
  const encMap = document.getElementById('encMap') as HTMLInputElement;
  if (encMap) {
    const encZone = document.getElementById('encZone') as HTMLInputElement;
