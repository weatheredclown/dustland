...
function renderInteriorList() {
  ...
  if (moduleData.interiors.length === 0) renderEmptyState(list, 'No interiors created yet.');
...
function setupListFilter(inputId, listId) {
  ...
  const apply = () => {
    ...
    Array.from(list.children).forEach(ch => {
      if ((ch as HTMLElement).classList.contains('list-empty-state')) return;
...
function renderNPCList() {
  ...
  if (npcs.length === 0) renderEmptyState(list, 'No NPCs created yet.');
  updateQuestOptions();
  refreshChoiceDropdowns();
  renderProblems();
}
...
function renderItemList() {
  ...
  if (items.length === 0) renderEmptyState(list, 'No items created yet.');
  refreshChoiceDropdowns();
  renderProblems();
}
...
function renderEncounterList() {
  ...
  if (moduleData.encounters.length === 0) renderEmptyState(list, 'No encounters created yet.');
}
...
function renderTemplateList() {
  ...
  if (moduleData.templates.length === 0) renderEmptyState(list, 'No templates created yet.');
  refreshChoiceDropdowns();
}
...
function renderEventList() {
  ...
  if (moduleData.events.length === 0) renderEmptyState(list, 'No tile events created yet.');
  populateFlagList();
}
...
function renderArenaList() {
  ...
  if (list.length === 0) renderEmptyState(listEl, 'No arenas created yet.');
}
...
function renderZoneList() {
  ...
  if (moduleData.zones.length === 0) renderEmptyState(list, 'No zones created yet.');
...
function renderPortalList() {
  ...
  if (moduleData.portals.length === 0) renderEmptyState(list, 'No portals created yet.');
}
...
function renderBldgList() {
  ...
  if (moduleData.buildings.length === 0) renderEmptyState(list, 'No buildings created yet.');
}
...
function renderQuestList() {
  ...
  if (moduleData.quests.length === 0) renderEmptyState(list, 'No quests created yet.');
  updateQuestOptions();
}
