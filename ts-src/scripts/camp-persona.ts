(function () {
  ...
  type GameSnapshot = DustlandGameSnapshot;

  const btn = document.getElementById('campBtn') as HTMLButtonElement | null;
  const overlay = document.getElementById('personaOverlay') as HTMLElement | null;
  const list = document.getElementById('personaList') as HTMLElement | null;
  const closeBtn = document.getElementById('closePersonaBtn') as HTMLButtonElement | null;
  const fastTravelBtn = document.getElementById('campFastTravelBtn') as HTMLButtonElement | null;
  const chestBtn = document.getElementById('campChestBtn') as HTMLButtonElement | null;
  const chestOverlay = document.getElementById('campChestOverlay') as HTMLElement | null;
  const chestList = document.getElementById('campChestList') as HTMLElement | null;
  const chestInvList = document.getElementById('campChestInventoryList') as HTMLElement | null;
  const closeChestBtn = document.getElementById('closeCampChestBtn') as HTMLButtonElement | null;
  ...
  function renderCampChest(): void {
    ...
    const chestItems = Array.isArray(chestItemsRaw)
      ? (chestItemsRaw as CampChestItem[])
      : [];
    chestList.innerHTML = '';
    ...
    const inv = Array.isArray(invRaw) ? (invRaw as CampChestItem[]) : [];
  ...
  function closeCampChest(restorePersona: boolean): void {
  ...
  if (chestBtn) {
    chestBtn.addEventListener('click', () => {
      ...
      const unlocked = inventory.isCampChestUnlocked?.();
      if (!unlocked) {
        const player = (globals.player ?? {}) as {
          scrap?: number;
          [key: string]: unknown;
        };
        ...
        if (scrap < CAMP_CHEST_COST) {
          ...
          if (typeof globals.toast === 'function') globals.toast('Not enough scrap.');
          return;
        }
        ...
        if (typeof globals.toast === 'function') globals.toast('Camp chest assembled.');
  ...
  bus.on('camp:open', () => {
    ...
    if (pos) {
      for (const z of zones) {
        if (z.if && !globals.checkFlagCondition?.(z.if)) continue;
    ...
    if (pos) {
      for (const memberState of pos) {
        if (typeof memberState.hydration === 'number') memberState.hydration = 2;
      }
    }
    globals.updateHUD?.();
    globals.log?.('You rest until healed.');
    const ensurePartyState = (): GameSnapshot | null => {
      const state = (gs.getState?.() as GameSnapshot | undefined) ?? null;
      if (!state) return null;
      if ((!Array.isArray(state.party) || state.party.length === 0) && Array.isArray(globals.party)) {
        gs.updateState?.((draft: DustlandGameSnapshot) => {
          if (globals.party) draft.party = globals.party;
        });
      }
      const refreshed = (gs.getState?.() as GameSnapshot | undefined) ?? state;
      return refreshed;
    };
    const state = ensurePartyState() ?? ({} as GameSnapshot);
    ...
    if (!ids.length) {
      ...
      ids.forEach(id => {
        const data = personas[id] ?? ({} as DustlandPersonaTemplate);
