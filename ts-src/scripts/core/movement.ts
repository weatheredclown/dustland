...
const activeMsgZones = new Set<Record<string, any>>();
...
function buildCombatant(def: Partial<CombatParticipant> & { HP?: number }, npc?: DustlandNpc): CombatParticipant {
  const base = def || {};
  ...
  return { ...base, hp, npc, name } as CombatParticipant;
}

function startMovementCombat(def: Partial<CombatParticipant>, npc?: DustlandNpc) {
...
function getEssentialRestockConfig() {
  ...
  if (essentialRestockCache.module !== moduleName) {
    ...
    const moduleData = movementGlobals.Dustland?.loadedModules?.[moduleName as string];
...
function restockEssentialSupplies(turn) {
  ...
  for (const [itemId, cfg] of config.entries()) {
    ...
    for (const npc of roster) {
      ...
      const shopInv = shop as { inv?: any[] };
      if (!Array.isArray(shopInv.inv)) shopInv.inv = [];
...
function zoneAttrs(map, x, y) {
  ...
  let minSteps = null;
  let maxSteps = null;
  const zones = (movementGlobals.Dustland?.zoneEffects ?? []) as DustlandZoneEffect[];
...
function onEnter(map, x, y, ctx) {
  ...
  const list = (Array.isArray(t.events) ? t.events : []).filter((ev: any) => ev.when === 'enter');
...
function applyZones(map, x, y) {
  ...
  for (const z of zones) {
    ...
    const step = (z.perStep || z.step) as Record<string, any> | undefined;
  ...
  if (canWalk(nx, ny)) {
    ...
    return new Promise<void>(resolve => {
      setTimeout(() => {
        ...
        if (pathApi?.tickPathAI) pathApi.tickPathAI();
...
function resolveEncounterChallenge(entry: DustlandEnemyBankEntry | undefined | null): number | null {
  if (!entry || typeof entry !== 'object') return null;
  if (Number.isFinite(entry.challenge)) return entry.challenge;
  const challenge = entry.combat && typeof entry.combat === 'object' && Number.isFinite((entry.combat as any).challenge)
    ? (entry.combat as any).challenge
    : null;
  ...
  if (templateId && typeof npcTemplates !== 'undefined' && Array.isArray(npcTemplates)) {
    ...
    if (tpl && tpl.combat && typeof tpl.combat === 'object' && Number.isFinite((tpl.combat as any).challenge)) {
      return (tpl.combat as any).challenge;
    }
  }
  return null;
}
...
function checkRandomEncounter() {
  ...
  const bank = enemyBanks[state.map] as Array<Record<string, any>> | undefined;
  ...
  if (Math.random() < chance) {
    let pool: Array<Record<string, any>> = bank.filter(entry => encounterMatchesLocation(entry, state.map, party.x, party.y, dist));
    ...
    if (Number.isFinite(guard)) {
      pool = pool.filter(entry => {
        ...
        return challenge >= (guard as number);
      });
      if (!pool.length) {
        encounterCooldown = minSteps + Math.floor(Math.random() * (maxSteps - minSteps + 1));
        return;
