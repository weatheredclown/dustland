...
(function () {
    ...
    function queue(map, start, goal, ignoreId) {
        ...
        if (state.cache.has(key) || state.queue.find(j => j.key === key))
            return key;
    ...
    function process() {
        ...
        setTimeout(() => {
            ...
            if (globalScope.perfStats)
                globalScope.perfStats.path += ((typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now()) - t0;
        ...
        while (open.length) {
            ...
            for (const nb of neighbors(current.x, current.y, map, ignoreId)) {
                ...
                if (tentative < (g[nk] ?? Infinity)) {
                    ...
                    if (!open.find(p => p.x === nb.x && p.y === nb.y))
                        open.push(nb);
                }
            }
        }
    ...
    function neighbors(x, y, map, ignoreId) {
        ...
        for (const [dx, dy] of dirs) {
            ...
            if (isWalkableAt(map, nx, ny, ignoreId))
                out.push({ x: nx, y: ny });
        }
        return out;
    }
    function isWalkableAt(map, x, y, ignoreId) {
        ...
        if (tiles && !tiles[String(info.tile)])
            return false;
    ...
    function tickPathAI() {
        const t0 = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
        const now = Date.now();
        const npcs = Array.isArray(globalScope.NPCS) ? globalScope.NPCS : [];
        ...
        for (const n of npcs) {
            ...
            if (n._lastMove && now - n._lastMove < npcMoveDelay)
                continue;
            const st = n._loop;
        ...
        if (globalScope.perfStats)
            globalScope.perfStats.ai += ((typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now()) - t0;
    }
    globalScope.Dustland = globalScope.Dustland || {};
    globalScope.Dustland.path = { queue, pathFor, tickPathAI, MAX_CACHE };
})();
