...
function setupNpcSections() {
    ...
    const persistSectionState = () => {
        try {
            const state = {};
            npcSectionRefs.forEach((el, id) => { state[id] = !!el.open; });
    ...
    sections.forEach(section => {
        ...
        if (saved !== undefined ? saved : section.open)
            detail.open = true;
...
function addEventSubBlock(container, ev = {}) {
    ...
    endMessages.placeholder = 'End messages, one per line (append ":: if flagName" for conditional slides)';
...
function mergeWizardResult(res) {
    ...
    if (typeof res.name === 'string') {
        const nameEl = document.getElementById('moduleName');
        if (nameEl)
            nameEl.value = res.name;
    }
    if (res.start && typeof res.start === 'object') {
        populateMapDropdown(document.getElementById('moduleStartMap'), res.start.map || 'world');
        const sx = document.getElementById('moduleStartX');
        const sy = document.getElementById('moduleStartY');
        if (sx)
            sx.value = String(res.start.x ?? '');
        if (sy)
            sy.value = String(res.start.y ?? '');
    }
...
try {
    if (typeof localStorage !== 'undefined' && !localStorage.getItem(START_HERE_KEY))
        showStartHere(true);
}
catch (e) { /* storage unavailable */ }
