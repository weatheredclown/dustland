...
var spawnHeatMax = 0;
function updateMapModeBanner(message) {
...
function beginCoordinatePick(target, message) {
    coordTarget = target;
...
function restoreSnapshot(snapshot) {
    ...
    buildings.length = 0;
    bldgCopy.forEach((b) => buildings.push(b));
...
function beginPlaceNPC() {
    ...
    placingCb = () => {
        if (editNPCIdx >= 0) {
            ...
            if (updated)
                npcOriginal = deepClone(updated);
        }
        else {
            captureSnapshot('Add NPC');
            const created = addNPC();
            if (created)
                npcOriginal = deepClone(created);
        }
        updateNPCValidation();
    };
    selectedObj = null;
...
function collectNPCFromForm() {
    ...
    const activeNpcIndex = (typeof npcDraftIndex === 'number' && npcDraftIndex >= 0) ? npcDraftIndex :
    ...
    const xField = document.getElementById('npcX');
    const yField = document.getElementById('npcY');
    ...
    if (combat) {
        ...
        if (cue || !Number.isNaN(dmgVal) || !Number.isNaN(delayVal)) {
            ...
            if (!Number.isNaN(dmgVal))
                npc.combat.special.dmg = dmgVal;
            if (!Number.isNaN(delayVal))
                npc.combat.special.delay = delayVal;
        }
    }
    if (shop)
...
function validateNPCForm() {
    ...
    const xVal = document.getElementById('npcX')?.value ?? '';
    const yVal = document.getElementById('npcY')?.value ?? '';
    const missing = [];
    const errors = [];
    if (!id)
        missing.push('id');
...
function updateNPCValidation(state) {
    ...
    const setInvalid = (el, invalid) => {
        ...
        if (invalid) {
            ...
            else if ('ariaInvalid' in el)
                delete el.ariaInvalid;
        }
    };
    ...
    if (pickBtn)
        pickBtn.disabled = result.missing.length > 0;
...
const undoBtn = document.getElementById('undo');
if (undoBtn)
    undoBtn.onclick = undoChange;
const redoBtn = document.getElementById('redo');
if (redoBtn)
    redoBtn.onclick = redoChange;
updateNPCValidation();
updateHistoryButtons();
...
canvas.addEventListener('mousedown', ev => {
    ...
    if (coordTarget) {
        ...
        document.getElementById(coordTarget.y).value = y;
        if (coordTarget.map)
            populateMapDropdown(document.getElementById(coordTarget.map), currentMap);
        finishCoordinatePick();
        canvas.style.cursor = '';
        drawWorld();
        return;
