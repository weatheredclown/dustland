...
function getWorldTileList() {
    if (typeof worldTileIds !== 'undefined' && Array.isArray(worldTileIds))
        return worldTileIds;
    const alt = globalThis.worldTileIds;
...
function getInteriorTileList() {
    if (typeof interiorTileIds !== 'undefined' && Array.isArray(interiorTileIds))
        return interiorTileIds;
    const alt = globalThis.interiorTileIds;
    if (Array.isArray(alt))
        return alt;
...
function cloneTileSpriteMap(map) {
...
function ensureTileGraphicsConfig(config) {
...
function clearSpriteCacheFor(path) {
    if (!path)
        return;
    delete spriteCache[path];
}
function getTileSpritePath(mapName, tile) {
    const graphics = moduleData.tileGraphics || {};
    const key = String(tile);
    const mapSprites = graphics.maps?.[mapName];
    if (mapSprites && mapSprites[key])
        return mapSprites[key];
    if (mapName !== 'world' && graphics.interiors && graphics.interiors[key])
        return graphics.interiors[key];
    if (graphics.defaults && graphics.defaults[key])
        return graphics.defaults[key];
    if (mapName === 'world' && graphics.maps?.world && graphics.maps.world[key])
        return graphics.maps.world[key];
    return undefined;
}
function getTileSprite(mapName, tile) {
    const path = getTileSpritePath(mapName, tile);
    if (!path)
        return null;
    return loadSprite(path);
}
function getNpcTileSpritePath(n) {
    if (!n)
        return undefined;
    if (typeof n.tileSprite === 'string' && n.tileSprite.trim())
        return n.tileSprite.trim();
    return undefined;
}
function getNpcTileSprite(n) {
...
const tileGraphicTarget = document.getElementById('tileGraphicTarget');
const tileGraphicMap = document.getElementById('tileGraphicMap');
...
function tileFallbackColor(tile, mapName) {
    if (mapName === 'world')
        return akColors[tile] || '#000';
...
function updateTilePaletteGraphics() {
    ...
    if (intPalette) {
        ...
        const tileMap = { W: TILE.WALL, F: TILE.FLOOR, D: TILE.DOOR };
        intPalette.querySelectorAll('button').forEach(btn => {
...
function getTileGraphicValue(scope, mapId, tileId) {
    const cfg = moduleData.tileGraphics || {};
    const key = String(tileId);
    if (scope === 'defaults')
        return cfg.defaults?.[key] || '';
    if (scope === 'interiors')
        return cfg.interiors?.[key] || '';
    if (scope === 'map')
        return cfg.maps?.[mapId]?.[key] || '';
    return '';
}
function setTileGraphicValue(scope, mapId, tileId, value) {
    ...
    if (scope === 'interiors') {
        if (value)
            cfg.interiors[key] = value;
        else
            delete cfg.interiors?.[key];
        return;
    }
    if (scope === 'map') {
        cfg.maps = cfg.maps || {};
        ...
        if (value)
            cfg.maps[mapId][key] = value;
        else {
            delete cfg.maps[mapId][key];
            if (Object.keys(cfg.maps[mapId]).length === 0)
                delete cfg.maps[mapId];
        }
    }
}
function renderTileGraphicsEditor() {
    ...
    const worldTiles = typeof getWorldTileList === 'function'
        ? getWorldTileList()
        : (Array.isArray(globalThis.worldTileIds)
            ? globalThis.worldTileIds
            : [
                typeof TILE !== 'undefined' && TILE.SAND != null ? TILE.SAND : 0,
                typeof TILE !== 'undefined' && TILE.ROCK != null ? TILE.ROCK : 1,
                typeof TILE !== 'undefined' && TILE.WATER != null ? TILE.WATER : 2,
    ...
    const interiorTiles = typeof getInteriorTileList === 'function'
        ? getInteriorTileList()
        : (Array.isArray(globalThis.interiorTileIds)
            ? globalThis.interiorTileIds
            : [
                typeof TILE !== 'undefined' && TILE.WALL != null ? TILE.WALL : 6,
    ...
    ids.forEach(id => {
        ...
        if (input.dataset) {
            ...
            input.dataset = { tile: String(id), scope, map: mapId };
        }
...
function updateTileGraphicScopeVisibility() {
    if (!tileGraphicTarget)
        return;
    if (tileGraphicMapWrap)
        tileGraphicMapWrap.style.display = tileGraphicTarget.value === 'map' ? 'block' : 'none';
}
...
function drawTileSprite(ctx, mapName, tile, px, py, sx, sy) {
...
function drawNpcMarker(ctx, npc, px, py, sx, sy, hovering) {
...
function updateNpcTilePreview() {
    ...
    const input = document.getElementById('npcTileSprite');
    const colorInput = document.getElementById('npcColor');
    if (!preview)
        return;
...
function startNewNPC() {
    ...
    const npcTileSprite = document.getElementById('npcTileSprite');
    if (npcTileSprite)
        npcTileSprite.value = '';
    updateNpcTilePreview();
...
function editNPC(i) {
    ...
    const npcTileSprite = document.getElementById('npcTileSprite');
    if (npcTileSprite)
        npcTileSprite.value = n.tileSprite || '';
    updateNpcTilePreview();
