...
function getAssetImage(id) {
    if (!moduleData.customImages || !moduleData.customImages[id])
        return null;
...
const moduleData = globalThis.moduleData ?? (globalThis.moduleData = { seed: Date.now(), name: 'adventure-module', npcs: [], items: [], quests: [], buildings: [], interiors: [], portals: [], events: [], zones: [], encounters: [], templates: [], personas: {}, start: { map: 'world', x: 2, y: Math.floor(WORLD_H / 2) }, module: undefined, moduleVar: undefined, props: {}, behaviors: {}, customImages: {}, tileOverrides: {} });
...
function renderAssetList() {
    ...
    list.querySelectorAll('.asset-paint').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const id = e.target.closest('.asset-row').dataset.id;
...
function injectAssetTab() {
    ...
    if (!tabList)
        return;
    if (document.getElementById('tabAssets'))
