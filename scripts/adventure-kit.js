...
function markNPCFieldInvalid(id, invalid) {
    ...
    if (label)
        label.classList.toggle('fieldError', invalid);
}
function updateNPCPlacementControls() {
    const cancelBtn = document.getElementById('npcPlacementCancel');
    const picking = placingType === 'npc' || coordTarget?.x === 'npcX';
    if (cancelBtn)
        cancelBtn.style.display = picking ? 'inline-block' : 'none';
    updateMapSelectionBanner();
}
...
function validateNPCForm() {
    ...
    const errors = [];
    if (!id)
        errors.push({ field: 'npcId', message: 'ID is required.' });
    const duplicate = id && moduleData.npcs.some((n, i) => n.id === id && i !== editNPCIdx);
    if (duplicate)
        errors.push({ field: 'npcId', message: 'ID already exists.' });
    if (!name)
        errors.push({ field: 'npcName', message: 'Name is required.' });
    if (!title)
        errors.push({ field: 'npcTitle', message: 'Title is required.' });
    const placementValid = !!map && Number.isFinite(x) && Number.isFinite(y);
    if (!placementValid)
        errors.push({ field: 'npcMap', message: 'Choose a tile on the map for this NPC.' });
    return {
        id,
        name,
        title,
...
function updateNPCValidation(state) {
    ...
    if (saveBtn)
        saveBtn.disabled = npcFormMode === 'idle' || !formState.valid;
    return formState;
}
function updateNPCActionBar() {
    const saveBtn = document.getElementById('saveNPC');
    if (saveBtn)
        saveBtn.textContent = npcFormMode === 'editing' ? 'Save Changes' : 'Save NPC';
    const discardBtn = document.getElementById('discardNPC');
    if (discardBtn)
        discardBtn.style.display = npcFormMode === 'idle' ? 'none' : 'inline-block';
}
function startNewNPC() {
    ...
    if (cancelPlacement)
        cancelPlacement.style.display = 'none';
