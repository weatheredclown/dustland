...
function exportSingleModule(file, silentIfMissing = false) {
...
function importSingleModule(file) {
    const modulePath = path.resolve(file);
    const baseName = path.basename(modulePath).replace(/\.module\.js$/, '');
    const jsonPath = path.join('data', 'modules', `${baseName}.json`);
    ...
    const text = fs.readFileSync(modulePath, 'utf8');
    const newText = text.replace(/const ((?:[A-Z0-9_]+_)?DATA) = `[\s\S]*?`;/, (_match, varName) => `const ${varName} = \`\n${cleanText}\n\`;`);
