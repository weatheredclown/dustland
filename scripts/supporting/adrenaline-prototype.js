...
function parseArgs(argv) {
    ...
    for (const raw of argv) {
        ...
        switch (key) {
            case 'hero-name':
                opts.heroName = value || opts.heroName;
                break;
            case 'hero-role':
                opts.heroRole = value || opts.heroRole;
                break;
            case 'hero-hp':
                opts.heroHp = toNumber(value, opts.heroHp);
                break;
            case 'hero-adr':
            case 'adr':
                ...
                break;
            case 'hero-adr-gen':
                opts.heroAdrGenMod = toNumber(value, opts.heroAdrGenMod);
                break;
            case 'hero-adr-dmg':
                opts.heroAdrDmgMod = toNumber(value, opts.heroAdrDmgMod);
                break;
            case 'hero-str':
                opts.heroStr = toNumber(value, opts.heroStr);
                break;
            case 'hero-agi':
                opts.heroAgi = toNumber(value, opts.heroAgi);
                break;
            case 'hero-luck':
                opts.heroLuck = toNumber(value, opts.heroLuck);
                break;
            case 'enemy-name':
                opts.enemyName = value || opts.enemyName;
                break;
            case 'enemy-hp':
                opts.enemyHp = toNumber(value, opts.enemyHp);
                break;
            case 'enemy-atk':
                opts.enemyAtk = toNumber(value, opts.enemyAtk);
                break;
            case 'enemy-challenge':
                opts.enemyChallenge = toNumber(value, opts.enemyChallenge);
                break;
            case 'delay':
                opts.turnDelay = toNumber(value, opts.turnDelay);
                break;
            case 'seed':
                opts.seed = value || opts.seed;
                break;
            case 'show-log':
                opts.showLog = true;
                break;
...
function prepareHero(options) {
    ...
    const party = adrenalineGlobal.party;
    if (party) {
        party.length = 0;
        party.push(hero);
    }
