const chiptuneGlobal = globalThis;
...
(function () {
    ...
    const toneWindow = isBrowser ? window : null;
    ...
    function toneWhen(t) {
        if (!audioCtx || !toneWindow?.Tone || typeof Tone.now !== 'function')
            return undefined;
    ...
    function playSnare(t, tight = false, vel = 1) {
        if (!audioCtx || !masterGain)
            return;
        if (tone.enabled && tone.ready && tone.synths) {
            const when = toneWhen(t);
    ...
    function resolvePresetId(id) {
        ...
        if (id && MOOD_PRESETS[id])
            return id;
        if (alias && MOOD_PRESETS[alias])
            return alias;
        return 'somber';
    }
    ...
    function setInstruments(opts) {
        if (!opts)
            return;
        if (opts.lead)
            instruments.lead = opts.lead;
        if (opts.bass)
            instruments.bass = opts.bass;
    }
