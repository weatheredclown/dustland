...
async function startCombat(defender) {
    ...
    const toEnemy = (def) => {
        ...
        return {
            ...
            prompt: (typeof prompt === 'string' ? prompt : npc?.prompt),
            special
        };
    };
    const enemies = [];
...
function setMap(id, label) {
    ...
    if (typeof centerCamera === 'function')
        centerCamera(party.x, party.y, state.map);
    if (id === 'world')
        setGameState(GAME_STATE.WORLD);
    else if (id === 'creator')
        setGameState(GAME_STATE.CREATOR);
    else
        setGameState(GAME_STATE.INTERIOR);
}
...
function applyZoneWalls(z) {
    if (!z || !z.walled || typeof setTile !== 'function')
        return;
    ...
    for (let i = 0; i < width; i++) {
        if (zoneGapContains(i, northGap) || (height === 1 && zoneGapContains(i, southGap)))
            continue;
    ...
    if (height > 1) {
        ...
        for (let i = 0; i < width; i++) {
            if (zoneGapContains(i, southGap))
                continue;
            setTile(map, baseX + i, by, wallTile);
        }
    }
    for (let i = 0; i < height; i++) {
        if (zoneGapContains(i, westGap) || (width === 1 && zoneGapContains(i, eastGap)))
            continue;
        ...
        if (id && globalThis.EventBus?.on) {
            globalThis.EventBus.on(`used:${id}`, () => {
                ...
                if (party.map !== map)
                    return;
                const { x, y } = party;
                if (x < z.x || y < z.y || x >= z.x + (z.w || 0) || y >= z.y + (z.h || 0))
                    return;
...
function mapSupportsFog(map) {
    ...
    if (interiorMaps && Object.prototype.hasOwnProperty.call(interiorMaps, map))
        return false;
    return true;
}
function ensureFogMap(map) {
    if (!mapSupportsFog(map))
        return null;
    state.fog = state.fog || {};
...
function setPartyPos(x, y) {
    ...
    if (mapSupportsFog(state.map))
        revealFog(state.map, party.x, party.y);
...
let lastInteract = 0;
const creatorMap = { id: 'creator', w: 30, h: 22, grid: [], entryX: 15, entryY: 10 };
function genCreatorMap() {
    creatorMap.grid = Array.from({ length: creatorMap.h }, (_, y) => Array.from({ length: creatorMap.w }, (_, x) => {
        const edge = y === 0 || y === creatorMap.h - 1 || x === 0 || x === creatorMap.w - 1;
        return edge ? TILE_SET.WALL : TILE_SET.FLOOR;
    }));
    interiors['creator'] = creatorMap;
}
function flagValue(flag) {
    if (worldFlags[flag])
        return worldFlags[flag].count;
...
function revealHiddenNPCs() {
    for (let i = hiddenNPCs.length - 1; i >= 0; i--) {
        ...
        if (checkFlagCondition(n.reveal)) {
            ...
            const tree = (typeof n.tree === 'object' && n.tree) ? n.tree : {};
...
function applyModule(data = {}, options = {}) {
    ...
    (dl.moduleProps || (dl.moduleProps = {}))[moduleName] = { ...(moduleData.props || {}) };
    (dl.loadedModules || (dl.loadedModules = {}))[moduleName] = moduleData;
    ...
    (moduleData.interiors || []).forEach(I => {
        ...
        if (finalLabel)
            interiorData.label = finalLabel;
        if (displayName)
            interiorData.displayName = displayName;
        interiors[id] = interiorData;
        if (displayName)
            mapLabels[id] = displayName;
    });
    if (moduleData.mapLabels) {
        Object.entries(moduleData.mapLabels).forEach(([key, value]) => {
            ...
            if (interior) {
                if (!cleanName(interior.displayName))
                    interior.displayName = label;
                if (!cleanName(interior.label))
    ...
    if (moduleData.portals)
        portals.push(...moduleData.portals);
    if (moduleData.events)
        registerTileEvents(moduleData.events);
    if (moduleData.zones)
        registerZoneEffects(moduleData.zones);
    if (moduleData.templates)
        npcTemplates.push(...moduleData.templates);
    if (moduleData.encounters) {
        const encounters = moduleData.encounters;
    ...
    if (typeof ITEMS !== 'undefined' && moduleData.items) {
        moduleData.items.forEach(it => {
            ...
            if (map !== undefined && x !== undefined && y !== undefined) {
                const loc = { id: registered.id, map: map || 'world', x, y, dropType: 'world' };
                itemDrops.push(loc);
    ...
    if (typeof log === 'function')
        log(`${nameForLog} loaded successfully.`);
    else
...
function genWorld(seed = Date.now(), data = {}) {
    ...
    for (let y = 0; y < WORLD_H; y++) {
        ...
        for (let x = 0; x < WORLD_W; x++) {
            ...
            if (v > .62)
                row.push(TILE_SET.ROCK);
            else if (v < -0.62)
                row.push(TILE_SET.WATER);
            else if (v > .18)
                row.push(TILE_SET.BRUSH);
            else
                row.push(TILE_SET.SAND);
        }
        world.push(row);
    }
    for (let x = 0; x < WORLD_W; x++) {
    ...
    for (let i = 0; i < 120; i++) {
        ...
        if (getTile('world', rx, ry) !== TILE_SET.WATER)
            setTile('world', rx, ry, TILE_SET.RUIN);
    }
    Object.keys(interiors).forEach(k => delete interiors[k]);
    if (creatorMap.grid && creatorMap.grid.length)
        interiors['creator'] = creatorMap;
    (data.interiors || []).forEach(I => { const { id, ...rest } = I; interiors[id] = { id, ...rest }; });
    buildings.length = 0;
    if (data.buildings) {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            if (getTile('world', x, y) === TILE_SET.WATER) {
                const p = findNearestLand(x, y);
                x = p.x;
                y = p.y;
            }
            placeHut(x, y);
...
function findNearestLand(sx, sy) {
    ...
    while (q.length) {
        ...
        for (const [dx, dy] of dirs) {
            ...
            const k = nx + "," + ny;
            if (nx >= 0 && ny >= 0 && nx < WORLD_W && ny < WORLD_H && !seen.has(k)) {
                seen.add(k);
                q.push([nx, ny]);
            }
        }
...
function placeHut(x, y, b = {}) {
    ...
    if (grid) {
        ...
        if (relY >= 0 && relY < h && relX >= 0 && relX < w)
            grid[relY][relX] = TILE_SET.DOOR;
        doorX = x + relX;
        doorY = y + relY;
    }
    ...
    for (let yy = 0; yy < h; yy++) {
        for (let xx = 0; xx < w; xx++) {
            const t = grid[yy][xx];
            if (t === TILE_SET.DOOR) {
                setTile('world', x + xx, y + yy, TILE_SET.DOOR);
            }
            else if (t === TILE_SET.BUILDING) {
                setTile('world', x + xx, y + yy, TILE_SET.BUILDING);
            }
        }
    }
    ...
    if (!bunker) {
        ...
        if (b.interiorId && !interiors[b.interiorId])
...
function resolveModuleName(rawModule) {
    ...
    function inner(value) {
        ...
        if (typeof value.id === 'string' && value.id)
            return value.id;
        if (typeof value.module === 'string' && value.module)
            return value.module;
...
function shopValuesEqual(key, current, base) {
    ...
    if (Array.isArray(currentVal) || Array.isArray(baseVal))
...
function normalizeShopValueForKey(value) {
    ...
    if (Array.isArray(value))
        return value.map(normalizeShopValueForKey);
    if (typeof value === 'object') {
        const out = {};
...
function save() {
    ...
    if (!Array.isArray(playerData?.inv))
        playerData.inv = [];
...
function loadLegacySave(d) {
    ...
    (d.npcs || []).forEach(n => {
        ...
        if (f) {
            ...
            if (n.portraitSheet)
                npc.portraitSheet = n.portraitSheet;
            if ('portraitLock' in n)
                npc.portraitLock = n.portraitLock;
            if (n.symbol)
                npc.symbol = n.symbol;
            if (n.trainer)
                npc.trainer = n.trainer;
            if (Array.isArray(n.loop))
                npc.loop = n.loop;
            if (n.quest) {
                if (quests[n.quest.id])
                    npc.quest = quests[n.quest.id];
                else if (npc.quest)
                    npc.quest.status = n.quest.status;
            }
...
function loadModernSave(d) {
    ...
    if (Array.isArray(d.world)) {
        ...
        if (moduleWorld.length && worldsDiffer(moduleWorld, d.world)) {
            ...
            if (discard) {
                ...
                if (typeof toast === 'function')
                    toast('Using updated module map.');
            }
        }
        if (useSavedWorld) {
            world.length = 0;
    ...
    (d.npcs || []).forEach(n => {
        ...
        if (f) {
            ...
            if (typeof n.locked === 'boolean')
                npc.locked = n.locked;
            if (typeof n.overrideColor === 'boolean')
                npc.overrideColor = n.overrideColor;
            if ('prompt' in n)
                npc.prompt = n.prompt;
            if (typeof n.questIdx === 'number')
                npc.questIdx = n.questIdx;
            if (n.quest) {
                if (quests[n.quest.id])
                    npc.quest = quests[n.quest.id];
                else if (npc.quest)
    ...
    if (typeof partyData.x === 'number')
        party.x = partyData.x;
    if (typeof partyData.y === 'number')
        party.y = partyData.y;
...
if (ccNext)
    ccNext.classList.add('btn--primary');
...
function openCreator() {
    if (!creator)
        return; // Gracefully skip when creator UI is absent
    if (!creatorMap.grid || creatorMap.grid.length === 0)
        genCreatorMap();
...
function renderStep() {
    ...
    if (step === 1) {
        ...
        nm.oninput = () => { building.name = nm.value; updateCreatorButtons(); };
        setCreatorPortrait();
    }
    if (step === 2) {
        ...
        r.querySelectorAll('button').forEach(b => b.onclick = () => { const k = b.dataset.k, d = parseInt(b.dataset.d, 10); if (d > 0 && pool <= 0)
            return; if (d < 0 && building.stats[k] <= 1)
            return; building.stats[k] += d; pool -= d; upd(); });
    }
    if (step === 3) {
        ...
        function updateSpecSelection(key) {
            ...
            if (descEl)
                descEl.textContent = spec?.desc || defaultSpecDesc;
        }
    ...
    if (step === 4) {
        ...
        function updateQuirkSelection(key) {
            ...
            if (descEl)
                descEl.textContent = quirk?.desc || defaultQuirkDesc;
        }
    ...
    if (step === 5) {
        ...
        const roll = rand(100);
        const got = roll < 12;
        building.origin = got ? 'Rustborn' : building.origin || null;
...
function finalizeCurrentMember() {
    ...
    if (!building.name || !building.name.trim())
        building.name = defaultDrifterName(built.length + 1);
    ...
    if (spec) {
        ...
        if (spec.gear) {
            spec.gear.forEach(g => {
                addToInv(g);
                if (['weapon', 'armor', 'trinket'].includes(g.type))
                    specEquipIds.push(g.id);
            });
        }
    }
...
function watchModuleLoad() {
    ...
    if (typeof document !== 'undefined' && document.addEventListener) {
        ...
        setTimeout(() => {
            if (!loaded)
                toast?.('Something went wrong loading the module. Check console.');
        }, 500);
    }
}
function beginGame() {
...
if (ccStart)
    ccStart.onclick = () => { if (built.length === 0) {
        finalizeCurrentMember();
    } closeCreator(); beginGame(); };
if (ccLoad)
    ccLoad.onclick = () => { load(); closeCreator(); };
if (creator?.addEventListener)
    creator.addEventListener('keydown', e => {
        if (e.key === 'Enter') {
            e.preventDefault();
            if (ccNext?.click)
                ccNext.click();
