(function () {
    ...
    const dl = (global.Dustland ?? (global.Dustland = {}));
    function ensureModule(info, cb) {
        ...
        if (globalName) {
            const mod = global[globalName] ?? null;
            if (mod) {
                const key = mod.name ?? moduleName ?? globalName;
                (dl.loadedModules ?? (dl.loadedModules = {}))[key] = mod;
                cb(mod);
                return;
            }
        }
        if (script) {
            ...
            el.src = script;
            el.onload = () => {
                const mod = globalName ? global[globalName] ?? null : null;
                if (mod) {
                    const key = mod.name ?? moduleName ?? globalName;
                    (dl.loadedModules ?? (dl.loadedModules = {}))[key] = mod;
                }
        ...
        mods.forEach(m => {
            ensureModule(m, moduleData => {
                if (moduleData) {
                    ...
                    if (moduleName) {
                        (dl.moduleProps ?? (dl.moduleProps = {}))[moduleName] = {
                            ...(moduleData.props ?? {}),
                            script: m?.script,
                            global: m?.global
                        };
    ...
    function open(fromId) {
        gatherBunkers(bunkers => {
            ...
            const fuel = global.player?.fuel ?? 0;
            ...
            moduleOrder.sort((a, b) => {
                if (a === originModuleKey)
                    return -1;
                if (b === originModuleKey)
                    return 1;
            ...
            moduleOrder.forEach((moduleName, idx) => {
                const columnNodes = (groups.get(moduleName) || []).slice().sort((a, b) => {
                    if (a.id === originNode?.id)
                        return -1;
                    if (b.id === originNode?.id)
                        return 1;
                ...
                const render = () => {
                    ...
                    if (!isOrigin) {
                        circle.onclick = () => {
                            ...
                            const fuelAvailable = global.player?.fuel ?? 0;
                            ...
                            if (b.id && confirm(`Travel to ${name} for ${cost} fuel?`))
                                travel(fromId, b);
                        };
                    }
    ...
    function travel(fromId, dest) {
        ...
        const ft = dl.fastTravel;
        ft?.saveSlot?.(fromId);
        if (!ft?.travel?.(fromId, dest.id))
            return;
        if (ft?.loadSlot?.(dest.id)) {
            close();
            return;
        }
