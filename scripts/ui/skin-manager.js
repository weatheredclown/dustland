...
(function () {
    ...
    if (!state.domReady) {
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (state.currentSkin)
                applySlotStyles(state.currentSkin);
        }, { once: true });
    }
    ...
    function normalizeExtension(ext, styleDir = '') {
        if (!ext)
            return styleDir.toLowerCase().includes('svg') ? '.svg' : '.png';
    ...
    function getPackagedGeneratedConfig(name) {
        ...
        if (direct)
            return cloneGeneratedConfig(direct);
    ...
    function mergeGeneratedConfig(base, override) {
        ...
        if (merged.manifest && typeof merged.manifest === 'object')
            merged.manifest = { ...merged.manifest };
        if (Array.isArray(merged.slots))
            merged.slots = [...merged.slots];
        else if (merged.slots && typeof merged.slots === 'object')
            merged.slots = { ...merged.slots };
        else
            merged.slots = merged.slots ?? null;
        return merged;
    }
    ...
    function extractManifestSections(manifest) {
        ...
        if (isPlainObject(manifest.ui)) {
            if (isPlainObject(manifest.ui.vars))
                result.uiVars = { ...manifest.ui.vars };
        ...
        if (manifest.tiles)
            result.tiles = manifest.tiles;
        if (manifest.icons)
            result.icons = manifest.icons;
        if (manifest.map)
            result.map = manifest.map;
        if (manifest.meta)
            result.meta = manifest.meta;
        if (typeof manifest.id === 'string')
            result.id = manifest.id.trim();
        if (typeof manifest.label === 'string')
            result.label = manifest.label.trim();
        return result;
    }
    function buildGeneratedSkin(name, config) {
        ...
        function appendDefaultExtension(base) {
            ...
            if (lower.endsWith(normalizedExtensionLower))
                return trimmed;
        ...
        if (tileEnum && typeof tileEnum === 'object') {
            for (const name of Object.keys(tileEnum)) {
                ...
                if (!tileKey || tileSlotDefinitions.has(tileKey))
                    continue;
        ...
        const ui = {};
        if (manifestSections.uiVars)
            ui.vars = { ...manifestSections.uiVars };
        if (Object.keys(slotStyles).length)
            ui.slots = slotStyles;
        let tileConfig = null;
        ...
        if (Object.keys(ui).length)
            skin.ui = ui;
        if (hasTileMapEntries(tileConfig))
            skin.tiles = tileConfig;
        if (manifestSections.icons)
            skin.icons = rewriteSpriteEntry(manifestSections.icons, baseDir, styleDir);
        if (manifestSections.map)
            skin.map = rewriteSpriteEntry(manifestSections.map, baseDir, styleDir);
        if (manifestSections.meta)
            skin.meta = { ...manifestSections.meta };
    ...
    function loadImage(src) {
        ...
        if (src && src.toLowerCase().endsWith('.svg')) {
            fetch(src)
                .then(response => {
                if (!response.ok)
                    throw new Error(`[SkinManager] HTTP error ${response.status} for ${src}`);
    ...
    function createSprite(entry, context) {
        ...
        const row = (() => {
            ...
            if (index != null) {
                ...
                const totalCols = columns || Math.floor((image.naturalWidth || image.width || 0) / cellWidth) || 0;
                if (totalCols > 0)
                    return Math.floor(index / totalCols);
            }
            return null;
        })();
        ...
        if (!Number.isFinite(spriteWidth) || !Number.isFinite(spriteHeight))
            return null;
        const sprite = {
            image,
            sx,
    ...
    function resolveTileDefinition(tiles, tileId) {
        ...
        if (typeof tileId === 'number') {
            ...
            if (name)
                keys.push(name, name.toUpperCase());
        }
    ...
    function cloneSpriteBase(entry) {
        ...
        for (const [key, value] of Object.entries(entry)) {
            ...
            if (isPlainObject(value))
                ...
                clone[key] = value.map(item => (isPlainObject(item) ? { ...item } : item));
            else
                clone[key] = value;
        }
        return clone;
    ...
    function collectEntityTags(entity) {
        ...
        if (!entity || typeof entity !== 'object')
            return tags;
        if (entity.id)
            tags.push(`id:${entity.id}`);
        if (entity.type)
            tags.push(entity.type);
        if (entity.role)
            tags.push(`role:${entity.role}`);
        if (entity.faction)
            tags.push(`faction:${entity.faction}`);
        if (entity.trainer)
            tags.push('trainer');
        if (entity.shop)
            tags.push('shop');
        if (entity.questId || entity.quests)
            tags.push('quest');
        if (entity.inanimate)
            tags.push('object');
        if (entity.attackOnSight || (entity.combat && !entity.friendly))
            tags.push('hostile');
        if (entity.friendly)
            tags.push('friendly');
        if (entity.combat)
        ...
        if (entity.tags && Array.isArray(entity.tags))
            tags.push(...entity.tags.map(String));
        return tags;
    }
    ...
    function prepareAssets(skin) {
        ...
        if (skin.tiles) {
            if (skin.tiles.atlas)
                loadImage(skin.tiles.atlas);
            const defs = skin.tiles.map || skin.tiles.tiles || {};
            for (const value of Object.values(defs))
                preloadTileDefinition(value);
        }
        if (skin.icons) {
            if (skin.icons.atlas)
                loadImage(skin.icons.atlas);
            ...
            if (skin.icons.extra)
                preloadIconGroup(skin.icons.extra);
        }
    }
    ...
    function getGeneratedSkinConfig(name) {
        ...
        if (stored)
            return cloneGeneratedConfig(stored);
