...
function canEquip(member, item) {
    if (!member || !item || !EQUIP_TYPES.includes(item.type))
        return false;
...
function storeCampChestItem(invIndex) {
    ...
    if (!Array.isArray(player.inv))
        player.inv = [];
    if (invIndex < 0 || invIndex >= player.inv.length)
        return false;
...
function tryAutoPickup(drop) {
    if (!drop || drop.dropType !== 'loot')
        return false;
    if (!player || !Array.isArray(player.inv))
        return false;
    if (!party)
        return false;
...
function equipItem(memberIndex, invIndex) {
    ...
    const it = player.inv[invIndex];
    if (!m || !it || !EQUIP_TYPES.includes(it.type)) {
