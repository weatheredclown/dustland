...
const _globals = globalThis;
const _getPlayer = () => {
    const current = _globals.player ?? (_globals.player = { inv: [], campChest: [], campChestUnlocked: false });
    if (!Array.isArray(current.inv)) {
        current.inv = [];
    }
    return current;
};
const _getParty = () => _globals.party;
const _getPartyState = () => _getParty();
const _ensureInventory = () => {
...
const _ITEMS = {};
const _itemDrops = [];
const _EQUIP_TYPES = ['weapon', 'armor', 'trinket'];
const _isEquipType = (value) => value === 'weapon' || value === 'armor' || value === 'trinket';
...
function _canEquip(member, item) {
    if (!member || !item || !_isEquipType(item.type))
        return false;
...
function _addExistingItemToInventory(item) {
    ...
    if (_isStackable(item)) {
        ...
        inventory.forEach((invItem, idx) => {
            if (!invItem || invItem.id !== item.id || !_isStackable(invItem))
                return;
...
function _mergeIntoCampChest(item) {
    ...
    for (const entry of chest) {
        if (!entry || entry.id !== item.id || !_isStackable(entry))
            continue;
...
function _isLootVacuumItem(it) {
    ...
    if (id && _LOOT_VACUUM_IDS.has(id))
        return true;
    const baseId = typeof it.baseId === 'string' ? it.baseId.toLowerCase() : '';
    if (baseId && _LOOT_VACUUM_IDS.has(baseId))
        return true;
    return false;
}
...
function _dropItemNearParty(item) {
    ...
    if (typeof toast === 'function')
        toast(`Inventory full, ${base.name} was dropped.`);
}
/**
...
function _tryAutoPickup(drop) {
    ...
    messages.forEach(msg => { if (typeof log === 'function')
        log(msg); });
...
function _addToInv(item) {
    ...
    if (stackable) {
        ...
        const stackPlan = [];
        inventory.forEach((invItem, idx) => {
            if (!invItem || invItem.id !== base.id || !_isStackable(invItem))
                return;
            const space = _getStackLimit(invItem) - _getStackCount(invItem);
            if (space <= 0)
                return;
...
function _removeFromInv(invIndex, quantity) {
    const inventory = _ensureInventory();
    if (invIndex < 0 || invIndex >= inventory.length)
        return;
...
function _equipItem(memberIndex, invIndex) {
    ...
    const it = inventory[invIndex];
    if (!m || !it || !_isEquipType(it.type)) {
        log('Cannot equip that.');
        return;
    }
    ...
    stats.forEach(stat => {
        ...
        if (diff)
            deltas.push(`${diff > 0 ? '+' : ''}${diff} ${stat}`);
    });
    _notifyInventoryChanged();
    log(`${m.name} equips ${it.name}.`);
    if (typeof toast === 'function') {
        toast(`${m.name} equips ${it.name}`);
        if (deltas.length)
            toast(deltas.join(', '));
    }
    _emit('sfx', 'tick');
...
function _useItem(invIndex) {
    ...
    if (it.use.type === 'heal') {
        ...
        log(msg);
        if (bonus > 0 && !it.use.text)
            log('Lucky boost!');
        if (typeof toast === 'function')
            toast(it.use.text || `${who.name} +${healed} HP`);
        _emit('sfx', 'tick');
        if (it.id === 'wand') {
    ...
    if (it.use.type === 'hydrate') {
        const who = (partyMembers[selectedMember] || partyMembers[0]);
        if (!who) {
            log('No party member to hydrate.');
            return false;
        }
        if (typeof who.hydration !== 'number')
            who.hydration = 0;
        ...
        log(msg);
        if (typeof toast === 'function')
            toast(it.use.text || `${who.name} +${who.hydration - before} HYD`);
        _maybeConsumeItem(it, invIndex);
    ...
    if (it.use.type === 'grenade') {
        ...
        if (enemies.length === 0) {
            log('You can only use that in combat.');
            return false;
        }
        const who = (partyMembers[selectedMember] || partyMembers[0]);
        if (!who) {
            log('No party member to throw that.');
            return false;
        }
        ...
        if (typeof aoe === 'function') {
            ...
            for (const target of enemies) {
                ...
                target.hp -= dealt;
                if (dealt > 0)
                    log?.(`${who.name}'s ${label} hits ${target.name} for ${dealt} damage.`);
                if (target.hp <= 0)
                    target.hp = 0;
            }
    ...
    if (it.use.type === 'cleanse') {
        const who = (partyMembers[selectedMember] || partyMembers[0]);
        if (!who) {
            log('No party member to cleanse.');
            return false;
        }
        ...
        log(msg);
        if (typeof toast === 'function')
            toast(it.use.text || `${who.name} is cleansed`);
        _emit('sfx', 'tick');
        _maybeConsumeItem(it, invIndex);
        _emit(`used:${it.id}`, { item: it });
        return true;
    ...
    if (Array.isArray(it.use.effects)) {
        for (const entry of it.use.effects) {
            ...
            if (typeof entry === 'string')
                effectList.push({ effect: entry });
            else if (typeof entry === 'object')
                effectList.push({ ...entry });
        }
    }
    if (it.use.effect) {
        if (typeof it.use.effect === 'string') {
            ...
            for (const [key, value] of Object.entries(it.use)) {
                if (key === 'effect' || key === 'effects' || key === 'consume' || key === 'text' || key === 'toast')
                    continue;
                extra[key] = value;
            }
