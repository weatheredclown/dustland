...
(function () {
    ...
    const inventoryEventBus = globalThis.Dustland?.eventBus ?? globalThis.EventBus;
    const emit = (event, payload) => {
        inventoryEventBus?.emit(event, payload);
    };
    const globals = globalThis;
    const getPlayer = () => {
        const current = globals.player ?? (globals.player = { inv: [], campChest: [], campChestUnlocked: false });
        if (!Array.isArray(current.inv)) {
            current.inv = [];
        }
        return current;
    };
    const getParty = () => globals.party;
    const getPartyState = () => getParty();
    ...
    const isEquipType = (value) => value === 'weapon' || value === 'armor' || value === 'trinket';
    ...
    function canEquip(member, item) {
        if (!member || !item || !isEquipType(item.type))
            return false;
    ...
    function addExistingItemToInventory(item) {
        ...
        if (isStackable(item)) {
            ...
            inventory.forEach((invItem, idx) => {
                if (!invItem || invItem.id !== item.id || !isStackable(invItem))
                    return;
    ...
    function mergeIntoCampChest(item) {
        ...
        for (const entry of chest) {
            if (!entry || entry.id !== item.id || !isStackable(entry))
                continue;
    ...
    function isLootVacuumItem(it) {
        ...
        if (id && LOOT_VACUUM_IDS.has(id))
            return true;
        const baseId = typeof it.baseId === 'string' ? it.baseId.toLowerCase() : '';
        if (baseId && LOOT_VACUUM_IDS.has(baseId))
            return true;
        return false;
    }
    ...
    function addToInv(item) {
        ...
        if (stackable) {
            ...
            const stackPlan = [];
            inventory.forEach((invItem, idx) => {
                if (!invItem || invItem.id !== base.id || !isStackable(invItem))
                    return;
                const space = getStackLimit(invItem) - getStackCount(invItem);
                if (space <= 0)
                    return;
