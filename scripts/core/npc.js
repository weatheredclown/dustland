...
class NPC {
    constructor({ id, map, x, y, color, name, title, desc, tree, quest = null, quests = null, questDialogs = null, processNode = null, processChoice = null, combat = null, shop = false, workbench = false, portraitSheet = null, portraitLock = true, symbol = '!', door = false, locked = false, prompt = null, unlockTime = null, questIdx = 0, trainer = null, overrideColor = false }) {
        ...
        if (!this.tree)
            this.tree = { start: { text: '', choices: [{ label: '(Leave)', to: 'bye' }] } };
        ...
        if (this.quest && !this.quest.status)
            this.quest.status = 'available';
        ...
        this.processNode = (node) => {
            if (this.quest)
                defaultQuestProcessor(this, node);
            capNode(node);
            if (userPN)
        ...
        if (userPC) {
            this.processChoice = (c) => { if (capChoice(c))
                return; return userPC.call(this, c); };
        }
        else {
...
function makeNPC(id, map, x, y, color, name, title, desc, tree, quest, processNode, processChoice, opts) {
    ...
    if (opts?.trainer) {
        ...
        if (!tree.train.choices?.length)
            tree.train.choices = [{ label: '(Back)', to: 'start' }];
    }
...
function createNpcFactory(defs) {
    ...
    (defs ?? []).forEach(n => {
        npcFactory[n.id] = (x = n.x, y = n.y) => {
            ...
            if (n.shop)
                opts.shop = cloneShopData(n.shop) ?? true;
            if (n.workbench)
                opts.workbench = true;
            if (n.portraitSheet)
            ...
            const npc = makeNPC(n.id, n.map ?? 'world', x, y, n.color, n.name ?? n.id, n.title ?? '', n.desc ?? '', tree, null, null, null, opts);
...
function scaleEnemy(npc, lvl = 1, build = []) {
    ...
    for (let i = 1; i < lvl; i++) {
        ...
        if (stat)
            npc.stats[stat] = (npc.stats[stat] ?? 0) + 1;
    }
    npc.lvl = lvl;
}
