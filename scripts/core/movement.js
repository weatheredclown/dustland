const movementGlobals = globalThis;
...
function getEssentialRestockConfig() {
    const moduleName = movementGlobals.Dustland?.currentModule;
    if (!moduleName)
        return null;
    if (essentialRestockCache.module !== moduleName) {
        ...
        const moduleData = movementGlobals.Dustland?.loadedModules?.[moduleName];
...
function restockEssentialSupplies(turn) {
    ...
    if (!config)
        return;
    const roster = Array.isArray(movementGlobals.NPCS) ? movementGlobals.NPCS : [];
    if (!roster.length)
        return;
    for (const [itemId, cfg] of config.entries()) {
        ...
        if (turn - lastTurn < interval)
            continue;
        if (roster.some(npc => shopHasItem(npc?.shop, itemId)))
            continue;
        const targetIds = Array.from(cfg.targets?.keys() || []);
        if (!targetIds.length)
            continue;
        for (const npc of roster) {
            if (!npc || !targetIds.includes(npc.id))
                continue;
            const shop = npc.shop;
            if (!shop || shop === true)
                continue;
            const shopInv = shop;
            if (!Array.isArray(shopInv.inv))
                shopInv.inv = [];
...
function markLootDropAges(beforeTurn) {
    ...
    for (const drop of itemDrops) {
        if (!isLootDrop(drop))
            continue;
        if (!Number.isFinite(drop.worldTurn))
            drop.worldTurn = beforeTurn;
    }
}
...
function zoneAttrs(map, x, y) {
    ...
    let minSteps = null;
    let maxSteps = null;
    const zones = (movementGlobals.Dustland?.zoneEffects ?? []);
    for (const z of zones) {
        ...
        if (z.noEncounters)
            noEncounters = true;
        if (typeof z.healMult === 'number')
            healMult *= z.healMult;
        if (z.spawns)
            spawns = z.spawns;
        if (typeof z.minSteps === 'number')
            minSteps = z.minSteps;
        if (typeof z.maxSteps === 'number')
            maxSteps = z.maxSteps;
    }
...
function calcMoveDelay(tile, actor, map = state.map) {
    ...
    if (delayMod)
        adjusted *= Math.max(0, 1 - delayMod);
...
function mapWH(map = state.map) {
    ...
    if (map === 'creator')
        return { W: creatorMap.w || VIEW_W, H: creatorMap.h || VIEW_H };
...
function getTile(map, x, y) {
    ...
    if (!g)
        return null;
    if (y < 0 || x < 0 || y >= g.length || !g[0] || x >= g[0].length)
        return null;
    return g[y][x];
}
function setTile(map, x, y, t) {
    ...
    if (!g)
        return false;
    if (y < 0 || x < 0 || y >= g.length || x >= g[0].length)
        return false;
    g[y][x] = t;
    return true;
}
...
function queryTile(x, y, map = mapIdForState()) {
    ...
    if (tile === null)
        return { tile: null, walkable: false, entities: [], items: [] };
...
function findFreeDropTile(map, x, y) {
    ...
    while (q.length) {
        ...
        for (const [dx, dy] of dirs) {
            ...
            if (nx >= 0 && ny >= 0 && nx < W && ny < H && !seen.has(k) && nd <= MAX_RAD) {
                seen.add(k);
                q.push([nx, ny, nd]);
            }
        }
...
function onEnter(map, x, y, ctx) {
    ...
    if (!t) {
        ...
        if (wasInDustStorm) {
            ...
            if (state.mapFlags)
                state.mapFlags.dustStorm = false;
        }
        return;
    }
    const list = (Array.isArray(t.events) ? t.events : []).filter((ev) => ev.when === 'enter');
    if (list.length)
        Effects.apply(list, ctx);
    const hasDustStorm = list.some(e => e.effect === 'dustStorm');
    if (state.mapFlags)
        state.mapFlags.dustStorm = hasDustStorm;
    if (!hasDustStorm) {
        Effects.apply([{ effect: 'dustStorm', active: false }]);
    }
}
...
function hasItemOrEquipped(idOrTag) {
    ...
    if (typeof hasItem === 'function' && hasItem(idOrTag))
        return true;
    if (typeof leader === 'function') {
        ...
        if (l && l.equip) {
            for (const sl of ['weapon', 'armor', 'trinket']) {
                const it = l.equip[sl];
                if (it && (it.id === idOrTag || (it.tags || []).map(t => t.toLowerCase()).includes(tag)))
                    return true;
            }
        }
    }
...
function applyZones(map, x, y) {
    ...
    for (const z of zones) {
        ...
        if (x < z.x || y < z.y || x >= z.x + (z.w || 0) || y >= z.y + (z.h || 0))
            continue;
        if (z.require && !hasItemOrEquipped(z.require))
            continue;
        if (z.negate && hasItemOrEquipped(z.negate))
            continue;
        if (z.weather)
            weatherZone = z;
        ...
        if (step && typeof step.hp === 'number') {
            ...
            renderParty?.();
            updateHUD?.();
            const baseMsg = typeof step.msg === 'string' ? step.msg : undefined;
            const msg = baseMsg || (delta > 0 ? 'You feel better.' : 'You take damage.');
            log?.(msg);
    ...
    if (weatherZone) {
        if (lastWeatherZone !== weatherZone) {
            if (!lastWeatherZone)
                prevWeather = globalThis.Dustland?.weather?.getWeather?.();
...
function applyStatusTicksOutOfCombat(actor) {
    ...
    for (const member of roster) {
        if (!member || !Array.isArray(member.statusEffects) || member.statusEffects.length === 0)
            continue;
        const beforeHp = member.hp;
        tick(member);
        if (member.hp !== beforeHp)
            changed = true;
    }
...
function move(dx, dy) {
    if (state.map === 'creator')
        return;
    if (typeof navigator !== 'undefined' && moveDelay > 0)
        return;
    ...
    if (canWalk(nx, ny)) {
        ...
        return new Promise(resolve => {
            setTimeout(() => {
                ...
                if (typeof footstepBump === 'function')
                    footstepBump();
                ...
                const pathApi = movementGlobals.Dustland?.path;
                if (pathApi?.tickPathAI)
                    pathApi.tickPathAI();
                if (state.map === 'world')
                    advanceWorldTurn();
                if (typeof movementGlobals.leaderHasLootVacuum === 'function' && movementGlobals.leaderHasLootVacuum()) {
                    let sweeps = 0;
...
function encounterMatchesLocation(entry, map, x, y, dist) {
    ...
    if (mode === 'zone') {
        ...
        for (const z of zones) {
            ...
            if (tag !== rawTag && id !== rawTag)
                continue;
            if (pointInZone(map, x, y, z))
                return true;
        }
        return false;
    }
    ...
    if (Math.random() < chance) {
        ...
        if (encounterBias) {
            ...
            const biased = pool.filter(e => {
                ...
                return tags.includes(tag) || id.includes(tag) || name.includes(tag);
            });
            if (biased.length)
                pool = biased;
        }
        ...
        const stats = movementGlobals.enemyTurnStats?.[id];
...
function interactAt(x, y) {
    ...
    if (info.items.length) {
        ...
        updateHUD();
        if (typeof pickupSparkle === 'function')
            pickupSparkle(x, y);
        bus.emit('sfx', 'pickup');
        return true;
    }
    if (x === party.x && y === party.y) {
        ...
        if (p) {
            if (p.if && !globalThis.checkFlagCondition?.(p.if)) {
                const blockedMsg = typeof p.blockedMsg === 'string' ? p.blockedMsg : 'It\'s locked.';
                log(blockedMsg);
                bus.emit('sfx', 'denied');
                return true;
            }
            ...
            if (state.map === 'world') {
                ...
                if (b.bunker) {
                    ...
                    if (typeof openWorldMap === 'function')
                        openWorldMap(b.bunkerId);
                    bus.emit('sfx', 'confirm');
                    return true;
                }
                const I = interiors[b.interiorId];
                if (I) {
                    setPartyPos(I.entryX, I.entryY);
                }
...
function interact() {
    if (Date.now() - lastInteract < 200)
        return false;
    lastInteract = Date.now();
    const dirs = [[0, 0], [1, 0], [-1, 0], [0, 1], [0, -1]];
    for (const [dx, dy] of dirs) {
        if (interactAt(party.x + dx, party.y + dy))
            return true;
    }
    return wait();
}
