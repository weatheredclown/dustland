    ...
    complete(outcome) {
        if (this.status !== 'completed') {
            ...
            if (typeof toast === 'function')
                toast(`QUEST COMPLETE: ${this.title}`);
...
function trackQuestDialogNode(npcId, nodeId) {
    ...
    Object.values(quests).forEach(q => {
        if (!q || q.status !== 'active' || !hasDialogGoals(q))
            return;
        const target = q.dialogNodes.find(goal => goal.npcId === npcId && (goal.nodeId || 'start') === nodeId);
        if (!target)
            return;
        if (!q.dialogProgress || typeof q.dialogProgress !== 'object')
            q.dialogProgress = {};
        const key = `${npcId}::${nodeId}`;
        if (q.dialogProgress[key])
            return;
...
function defaultQuestProcessor(npc, nodeId) {
    ...
    if (meta.progress >= requiredCount && hasFlag) {
        ...
        if (Array.isArray(npc.quests)) {
            ...
            if (npc.quest && !npc.quest.status)
                npc.quest.status = 'available';
    ...
    if (dialogGoal) {
        if (meta.progressText)
            message = meta.progressText;
        else
            message = `You still need to finish earning that favor. (${progress}/${requiredCount})`;
    }
    else if (itemKey) {
    ...
    if (typeof textEl !== 'undefined' && textEl)
