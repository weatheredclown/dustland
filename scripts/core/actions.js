        ...
        applyQuestReward(reward) {
            ...
            if (typeof reward === 'string' && /^xp\s*\d+/i.test(reward)) {
                ...
                if (typeof updateHUD === 'function')
                    updateHUD();
                if (typeof globals.toast === 'function')
                    globals.toast(`+${amt} ${globals.CURRENCY || 'Scrap'}`);
            }
            else {
                ...
                if (item) {
                    if (typeof globals.addToInv === 'function') {
                        if (!globals.addToInv(item)) {
                            if (typeof globals.dropItemNearParty === 'function')
                                globals.dropItemNearParty(item);
                        }
                        else {
                            if (typeof globals.toast === 'function')
                                globals.toast(`Received ${item.name}`);
                        }
                    }
                }
            }
