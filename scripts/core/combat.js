...
window.setBossTelegraphFX = (opts = {}) => {
    if (typeof opts.intensity === 'number')
        window.bossTelegraphFX.intensity = opts.intensity;
    if (typeof opts.duration === 'number')
        window.bossTelegraphFX.duration = opts.duration;
};
if (cmdMenu) {
...
function recordCombatEvent(ev, relay = true) {
    ...
    if (relay)
        globalThis.EventBus?.emit('combat:event', ev);
}
...
function hasStatusImmunity(target, type) {
    ...
    if (target.equip) {
        ...
        for (const slot of slots) {
            ...
            if (tags.some(t => typeof t === 'string' && t.toLowerCase() === key))
                return true;
            const mods = it.mods || {};
            if (mods[key] || mods[`${type}Immune`])
                return true;
        }
    }
    return false;
...
function tickStatuses(target) {
    if (!target || !Array.isArray(target.statusEffects))
        return false;
    for (let i = target.statusEffects.length - 1; i >= 0; i--) {
        ...
        if (s.remaining <= 0) {
            ...
            if (s.type === 'poison')
                log?.(`${target.name} is no longer poisoned.`);
        }
    }
    return target.hp <= 0;
}
...
function renderCombat() {
    ...
    for (const e of combatState.enemies) {
        ...
        adr.className = 'hudbar adr';
        adr.style.width = '48px';
...
function openCombat(enemies) {
    if (!combatOverlay)
        return Promise.resolve({ result: 'flee' });
    return new Promise((resolve) => {
        for (const k in combatKeys)
            combatKeys[k] = false;
        combatState.turns = 1;
    ...
    if (result === 'bruise') {
        if (state.mapEntry) {
            ...
            if (typeof toast === 'function')
                toast('You wake up at the entrance.');
...
function openCommand() {
    if (combatState.phase !== 'party' || !cmdMenu)
        return;
    const m = party[combatState.active];
    if (m && tickStatuses(m)) {
    ...
    ['Attack', 'Special', 'Item', 'Flee'].forEach((opt) => {
        ...
        d.textContent = opt;
        if (opt === 'Special' && !hasSpecial)
            d.classList.add('disabled');
        if (opt === 'Item' && (!(player?.inv?.length > 0)))
            d.classList.add('disabled');
...
function handleCombatKey(e) {
    if (!combatOverlay || !combatOverlay.classList.contains('shown'))
        return false;
    if (e.repeat && !combatKeys[e.key])
        return false;
...
function labelRequirement(entry) {
    ...
    if (entry && typeof entry === 'object') {
        if (entry.name)
            return entry.name;
        if (entry.id)
            return labelRequirement(entry.id);
    }
    return humanizeRequirementId(entry);
}
...
function matchesWeaponRequirement(entry, weapon) {
    ...
    if (typeof entry === 'string') {
        ...
        if (!weapon)
            return false;
        const id = entry;
        if (weapon.id === id || weapon.baseId === id)
            return true;
        return false;
    }
    if (entry && typeof entry === 'object') {
        if (entry.id)
            return matchesWeaponRequirement(entry.id, weapon);
        if (entry.tag)
            return matchesWeaponRequirement(`tag:${entry.tag}`, weapon);
...
function evaluateWeaponRequirements(requirements, weapon, mode = 'any') {
    ...
    for (const entry of entries) {
        if (matchesWeaponRequirement(entry, weapon))
            matched.push(entry);
        else
            missing.push(entry);
    }
...
function doAttack(dmg, type = 'basic') {
    ...
    for (const target of targets) {
        ...
        if (tDmg > 0)
            log?.(`${attacker.name} hits ${target.name} for ${tDmg} damage.`);
...
function handleEnemyDefeat(attacker, target, sourceLabel) {
    ...
    if (typeof target.scrap === 'object' && Math.random() < (target.scrap.chance ?? 1)) {
        ...
        if (amt > 0) {
            ...
            if (luckyLint)
                log?.('Lucky Lint shakes loose extra scrap.');
        }
    }
    ...
    if (target.boss && Math.random() < 0.1) {
    ...
    if (killerHasBrutalPast) {
        ...
        const notes = [];
        if (healed > 0)
            notes.push(`+${healed} HP`);
        if (adrGain > 0)
            notes.push(`+${adrGain} ADR`);
        if (notes.length) {
...
function applyDeathEffects(target) {
    ...
    if (Array.isArray(fx) && fx.length)
        globalThis.Dustland?.effects?.apply?.(fx);
}
...
function enemyAttack() {
    ...
    if (tickStatuses(enemy)) {
        ...
        if (enemy.loot && Math.random() < (enemy.lootChance ?? 1))
            addToInv?.(enemy.loot);
