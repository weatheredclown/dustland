...
- **Last updated**: 2025-11-19 by automated agent.
- **Files remaining with `@ts-nocheck`**: 5 (`scripts/core/combat.ts`, `scripts/core/movement.ts`, `scripts/dustland-core.ts`, `scripts/dustland-engine.ts`, `scripts/adventure-kit.ts`). The count is generated by `rg "@ts-nocheck" ts-src -l | wc -l` and should trend downward after every milestone.
