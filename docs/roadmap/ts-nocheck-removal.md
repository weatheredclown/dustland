...
- **Last updated**: 2025-10-30 by automated agent.
- **Files remaining with `@ts-nocheck`**: 18 (9 core engine, 5 module bundles, 4 UI/tooling scripts). The count is generated by `rg "@ts-nocheck" ts-src -l | wc -l` and should trend downward after every milestone.
