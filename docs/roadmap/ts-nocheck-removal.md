...
- **Last updated**: 2025-11-29 by automated agent.
- **Files remaining with `@ts-nocheck`**: 1 (`ts-src/scripts/adventure-kit.ts`). The count is generated by `rg "@ts-nocheck" ts-src -l | wc -l` and should stay at zero once the Adventure Kit is typed.
...
- **Primary risks**: the Adventure Kit still leans on `globalThis` state, ad-hoc event payloads, and duplicate literal schemas across modules. The focused plan below keeps the churn manageable while closing the final gap.
...
**Status**
- Partially achieved; `dustland-nano.ts`, `module-picker.ts`, and multiplayer surfaces compile cleanly. The remaining suppression lives in the Adventure Kit.
...
## 4. Adventure Kit `@ts-nocheck` removal (in flight)
