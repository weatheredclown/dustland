...
- **Files remaining with `@ts-nocheck`**: 6 (core engine focus areas, module bundles, and UI/tooling scripts). The count is generated by `rg "@ts-nocheck" ts-src -l | wc -l` and should trend downward after every milestone.
