...
**Reference implementation: PR #1562 (complete)**
- **Schema:** `CustomAssetMeta` and `TileOverride` types defined, with `customAssets` and `tileOverrides` added to `ModuleData`.
- **Engine:** Render loop draws layered tile overrides with transparency preserved and caches decoded assets for reuse.
- **Editor:** Added a dedicated **Paint** tab that supports uploads, selection, and clearing of per-tile overrides.
- **NPCs/Items:** `tileSprite` fields in the editors resolve to custom asset IDs alongside existing palette skins.

### Notable implementation details from PR #1562
- **Editor safety rails:** Painting is gated to the Paint tab with explicit modes (asset vs. clear) so standard map editing clicks do not accidentally place overrides. The `mousedown` handler checks the active tab before starting a paint drag, preventing conflicts with selection.
- **Override lifecycle:** `tileOverrides` are stored per map (`tileOverrides[mapId]["x,y"]`), letting the editor clear or swap overrides on the active map without touching interiors or other zones.
- **Rendering & cache:** Overrides draw after base tiles with alpha preserved, and the engine caches `ImageBitmap`s by asset ID so repainting the same custom tile across many coordinates avoids re-decoding.
